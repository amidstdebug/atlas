...
class OnlinePipeline:
    ...
    def _get_sliding_window_feature(self, waveform: np.ndarray, audio_start_time: float):
    ...
    def _annotate(self, waveform: np.ndarray, audio_start_time: float):
        waveform = self._get_sliding_window_feature(waveform, audio_start_time)
    ...
    def __call__(self, waveform: Union[torch.Tensor, np.ndarray], sample_rate: int = 16_000):
        ...
        while self.waveform.shape[0] > (self.last_processed_start_time + self.duration):
            ...
            self._annotate(audio, audio_start_time)
