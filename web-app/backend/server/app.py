...
import jwt
import datetime
from fastapi import FastAPI, File, UploadFile, HTTPException, Header, WebSocket, Depends, BackgroundTasks
...
app = FastAPI(title="Meeting Minutes Transcription API", version="1.0.0")
...
async def summarize_text(transcription: str, previous_report: Optional[str] = None, summary_mode: str = 'minutes') -> Dict:
	try:
		...
		# Get the system prompt from the appropriate
...
async def websocket_endpoint(websocket: WebSocket):
	...
	connected_clients[client_id] = {
		'user_id': 'anonymous',  # Simple mode without authentication for demo
	...
	try:
		while True:
			...
			if 'audio' in data:
				...
				if not audio_base64 or chunk_id is None:
					await websocket.send_json({'error': 'Missing required data'})
					continue
				
				# Track this chunk as pending
				connected_clients[client_id]['pending_chunks'].add(chunk_id)
				
				# Send to audio server for transcription
				payload = {
					'audio': audio_base64,
					'chunk_id': chunk_id,
				...
				# Make request to audio server
				response = requests.post(f"{AUDIO_SERVER_URL}/transcribe/base64", json=payload)
				
				if response.status_code != 200:
					await websocket.send_json({
						'chunk_id': chunk_id,
						'error': f"Failed to transcribe chunk {chunk_id}"
					})
					
					connected_clients[client_id]['pending_chunks'].discard(chunk_id)
					continue
				...
				# Store in buffer for summary generation
				connected_clients[client_id]['transcription_buffer'].append(transcription)
				connected_clients[client_id]['pending_chunks'].discard(chunk_id)
				
				# Generate summary if buffer is large enough
				if len(connected_clients[client_id]['transcription_buffer']) >= 5:
					combined_text = " ".join(connected_clients[client_id]['transcription_buffer'])
					
					# Generate summary without authentication
					try:
						summary_result = await summarize_text(combined_text, None, 'minutes')
						
						# Send summary to client
						await websocket.send_json({
							'summary': summary_result.get('message', {}).get('content', ''),
							'timestamp': datetime.datetime.now().isoformat()
						})
						
						# Clear buffer
						connected_clients[client_id]['transcription_buffer'] = []
					except Exception as e:
						logger.error(f"Error generating summary: {str(e)}")
						await websocket.send_json({'error': f"Summary generation error: {str(e)}"})
...
async def lifespan(app: FastAPI):
	...
	try:
		...
		logger.warning(f"Could not connect to audio server: {str(e)}")
	...
	try:
		...
		ollama_response = requests.post(LLM_URI, json=payload, timeout=5)
